...
void main()
{
	...
	int atlasTextureIndex = texelFetch(nodeIndirectionSampler, node_indirection_base_idx + normalizedCoordToLinearOffset(pos)).r;
	...
	if (atlasTextureIndex != -1) {
		...
		soot = texture3D(soot_texture, coords).rrrr;

		soot.xyz = vec3(atlasTextureIndex);
	}
