									...
									ParticleMesherModifierData *pmmd, Scene *scene)
{
	...
	if (cutter_dm != NULL) {
		...
		mesher_mask = OpenVDB_from_polygons(mask_geom, mesher_mask, pmmd->voxel_size, pmmd->int_band, pmmd->ext_band);
	}
	...
	if (cutter_dm != NULL) {
		OpenVDBPrimitive_free(mesher_mask);
