...
static void paint_2d_lift_smear(ImBuf *ibuf, ImBuf *ibufb, int *pos, short tile, short lock_alpha)
{
...
static int paint_2d_op(void *state, ImBuf *ibufb, unsigned short *curveb, unsigned short *texmaskb, const float lastpos[2], const float pos[2])
{
	...
	const bool locked = ((ImageLayer *)BKE_image_get_current_layer(s->image))->locked;
	...
	if (s->tool == PAINT_TOOL_SOFTEN) {
		...
		paint_2d_lift_smear(s->canvas, ibufb, blastpos, tile, locked);
	}
		...
		clonebuf = paint_2d_lift_clone(s->clonecanvas, ibufb, bliftpos, locked);
	}
	...
	for (a = 0; a < tot; a++) {
		...
		if (s->do_masking) {
			...
			for (ty = tiley; ty <= tileh; ty++) {
				for (tx = tilex; tx <= tilew; tx++) {
					...
					IMB_rectblend(s->canvas, tmpbuf, frombuf, mask,
					              ...
					              origx, origy,
					              region[a].srcx, region[a].srcy,
								  region[a].width, region[a].height, blend, ((s->brush->flag & BRUSH_ACCUMULATE) != 0), locked);
				}
			}
			...
			IMB_rectblend(s->canvas, s->canvas, frombuf, NULL, curveb, texmaskb, mask_max,
			              ...
			              region[a].destx, region[a].desty,
			              region[a].srcx, region[a].srcy,
						  region[a].width, region[a].height, blend, false, locked);
		}
	}
...
{
	BKE_image_release_layer_ibuf(s->canvas);
...
void paint_2d_stroke(void *ps, const float prev_mval[2], const float mval[2], const bool eraser, float pressure, float distance, float size)
{
	...
	ImBuf *ibuf = BKE_image_acquire_layer_ibuf(s->image);
