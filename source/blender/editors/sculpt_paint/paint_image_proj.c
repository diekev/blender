	...
	if (LIKELY(accum_tot != 0)) {
		...
		if (ps->mode == BRUSH_STROKE_INVERT) {
			...
			if (fabsf(rgba[0]) > ps->brush->sharp_threshold) {
				...
				blend_color_add_float_v4(rgba, projPixel->pixel.f_pt, rgba, rgba[3]);
				rgba[3] = alpha;
			}
			else
				return;
	...
	if (LIKELY(accum_tot != 0)) {
		...
		if (ps->mode == BRUSH_STROKE_INVERT) {
			...
			if (fabsf(rgba[0]) > ps->brush->sharp_threshold) {
				...
				/* add to enhance edges */
				blend_color_add_float_v4(rgba, rgba_pixel, rgba, rgba[3]);
				...
				if (ps->source == PROJ_SRC_VIEW_FILL) {
					...
					if (is_floatbuf) {
						...
						if (projPixel->newColor.f[3]) {
							...
							blend_color_mix_float_v4(projPixel->pixel.f_pt,  projPixel->origColor.f_pt,
							                         projPixel->newColor.f, projPixel->newColor.f[3]);
						}
					}
					else {
