                                 ...
                                 bool *use_portal)
{
	...
	if(object_has_level_set(b_ob)) {
		...
		/* 0 - single, 1 - sequence */
		int is_sequence = RNA_enum_get(&cyc, "sequence" );
		if(is_sequence == 1) {
		...
		if(abs_path != object->levelset_path || levelset_material != object->levelset_material_slot) {
	...
	if(object->levelset_path != "") {
		if(levelset_map.sync( &levelset, b_ob, b_parent, levelset_key))
			level_set_updated = true;
	}

	if (level_set_updated) {
		int shader_id = 0;
